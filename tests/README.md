...
The application uses a sophisticated view management system to preserve computed data during navigation (browser back/forward) and layout changes. Understanding this is essential for writing reliable tests.

### View and Board Relationship

- **View** (main thread): Holds computed pixel data (`nn` array), coordinates, and display state
- **Board** (worker thread): Performs the actual Mandelbrot computation for a view
- **Scheduler**: Manages boards across workers, tracks which board is at each index

Each view has a unique `id` (e.g., `0.5108225063544805`) that links it to its board in the worker.

### The stableViews System

When the layout changes (popstate, grid resize, etc.), views need to be preserved:
...
1. **stableViews is null except during updates**: If `stableViews` is non-null, an update is in progress
2. **Worker messages route to stableViews during updates**: If `this.views[k]` is empty, messages are routed to matching views in `stableViews` by ID
3. **Boards are tracked by ID, not just index**: The scheduler's `boardIds` map prevents removing the wrong board when indices change
4. **processingPopState flag prevents forward history loss**: During popstate handling, `updateurl()` doesn't push new history entries

### Race Condition Prevention

The system handles several race conditions:

1. **Worker message during view clear**: When `saveViewsToStable()` clears `this.views`, worker messages still reach views via `stableViews`
2. **Grid column changes**: When pressing H/G, dimensions change. Old boards are removed only if their ID matches the current board at that index
3. **Rapid back/forward**: Multiple popstate events are handled correctly because stableViews preserves the complete view pool

### Testing Navigation Behavior

When testing browser history and navigation:
...
// Verify stableViews is cleaned up
const stableViews = await page.evaluate(() =>
  window.explorer.grid.stableViews
);
expect(stableViews).toBeNull();

// Verify computation continues
const beforeDi = await page.evaluate(() =>
  window.explorer.grid.views.map(v => v.di)
);
await page.waitForTimeout(2000);
const afterDi = await page.evaluate(() =>
  window.explorer.grid.views.map(v => v.di)
);
expect(afterDi[0]).toBeGreaterThan(beforeDi[0]);
```
