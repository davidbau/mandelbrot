...
describe('History Fuzzing Stress Tests', () => {
  ...
  test('30 seconds of hide/unhide cycling', async () => {
    ...
    // Check max iteration - if very high (>1M), remaining pixels may legitimately be slow
    const maxIterations = await page.evaluate(() =>
      Math.max(...window.explorer.grid.views.map(v => v ? v.it : 0))
    );
    const isDeepComputation = maxIterations > 1000000;

    // Only report as stalled if there was unfinished work but no progress
    // Exception: if max iteration is very high, remaining chaotic pixels can legitimately take minutes
    if (!anyProgressed && hasUnfinishedWork && !isDeepComputation) {
      ...
      const diagnostics = await page.evaluate(() => {
        ...
        return {
          ...
          stableViewsExists: !!window.explorer.grid.stableViews,
          stableViewsLen: window.explorer.grid.stableViews?.length,
          currentUpdateProcess: !!window.explorer.grid.currentUpdateProcess
        };
      });
